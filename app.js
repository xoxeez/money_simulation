...
function calcHouse() {
    ...
    const setBal = (id, descId, val) => {
        $(id).textContent = eok(val); $(id).className = "v " + (val < 0 ? "short" : "ok");
...
function calcLiving() {
    ...
    expenses.forEach(e => {
        const a = +e.amt || 0; const tr = document.createElement("tr");
...
function initFirebase() {
    ...
    if (String(firebaseConfig.apiKey).startsWith("여기에")) { setStatus("Firebase 미설정", "err"); return; }
    try {
        ...
        firebase.auth().signInAnonymously()
            .then(() => {
                setStatus("Firebase 연결됨 (로그인) · " + firebaseConfig.projectId, "on");
                loadFromCloud(true);
            })
            .catch((e) => { console.error(e); setStatus("로그인 실패: " + e.message, "err"); });
