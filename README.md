...
거래 화면에서 수입, 지출, 계좌 이체를 1회성 또는 매월 정기 항목으로 등록할 수 있습니다. 정기 항목은 시작 월부터 월별 화면에 예정으로 표시되고, 해당 월에 실제 발생했을 때 완료로 바꾸거나 발생하지 않은 달은 건너뛸 수 있습니다. 완료된 정기 지출만 실제 지출 합계에 반영해 예정액과 중복 집계하지 않습니다. 정기 항목을 수정하면 선택한 월부터 새 규칙이 적용되고, 삭제도 선택한 월 이후의 반복만 종료해 앞선 월 기록은 유지합니다. 개별 거래를 눌러 수정하거나 삭제할 수 있고, 실제 카드 지출은 기존 정산 검토 흐름과 연결됩니다.
